...
def main_worker(gpu, ngpus_per_node,args):
    ...
    if os.path.exists(pretrain_path) and os.path.isfile(pretrain_path) and os.path.getsize(pretrain_path) > 1000:
        ...
        checkpoint = torch.load(pretrain_path, map_location='cpu')
        checkpoint_model = checkpoint['model']
        # Create a copy and remove encoder/vit_backbone keys since they're already loaded into vit_backbone
        decoder_checkpoint = {k: v for k, v in checkpoint_model.items() 
                             if not k.startswith('vit_backbone.')}
        interpolate_pos_embed_inputsize(model, decoder_checkpoint,
                                        input_size=patch_wise_size,use_decoder=True)
        msg = model.load_state_dict(decoder_checkpoint, strict=False)
