...
class Generator40x40(nn.Module):
    """
    Wrapper for HiCSR Generator to handle 40x40 input/output.
    ...
    Note: HiCSR outputs tanh (range [-1, 1]), which will be handled by output adapter.
    """
    def __init__(self, num_res_blocks=15):
        super(Generator40x40, self).__init__()
        self.generator = Generator(num_res_blocks=num_res_blocks)
    ...
    def forward(self, x):
        ...
        # Pass through original generator (outputs tanh, range [-1, 1])
